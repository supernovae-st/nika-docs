...
- name: summarize
  infer:
    model: anthropic/<model-id>          # <provider>/<name> · see /reference/providers-catalog
...
- `AgentLoopConfig` bounds the loop: `max_turns`, `max_tokens_total`.
  The first ceiling hit stops the loop (`AgentStopReason`).
