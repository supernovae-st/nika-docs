---
title: "The five verbs"
description: "exec · fetch · invoke · infer · agent. Five verbs, closed set, typed contracts. Everything else is data."
sidebarTitle: "Verbs"
icon: "play"
---

import { STATUS } from "/snippets/_status-snapshot.mdx";

A Nika workflow is a list of **tasks**. Each task picks exactly one of
**five verbs** — `exec`, `fetch`, `invoke`, `infer`, `agent` — and
supplies its required fields. The set is closed: the parser's verb
dispatcher is exhaustive at compile time, so adding a sixth verb is a
schema-version bump, not a patch release.

<Info>
  **Target design banner.** Verb grammar is locked by `nika-schema` raw
  AST (`RawAction` enum). The YAML parser is WIP — scaffolding lives at
  `crates/nika-schema/` and graduates in **Round 4** admission. Snippets
  below reflect the target shape, not what executes end-to-end at v{STATUS.version}.
</Info>

## Why five?

Every workflow decomposes into one of these moves:

| Verb | One-line intent |
|---|---|
| `exec` | Run a local command. |
| `fetch` | Pull data from the world (HTTP/URL). |
| `invoke` | Call an MCP tool or resource. |
| `infer` | Ask an LLM. |
| `agent` | Delegate to an agent loop (LLM + tools + iteration budget). |

Anything else we considered — "transform", "map", "branch", "retry" —
was a special case of one of these five, or workflow-level machinery
(dependencies, `for_each`, `condition`) that lives on the task envelope,
not as a new verb.

## `exec` — run a local command

Spawns a subprocess, captures stdout/stderr, returns exit code + output.

**Required**: `command` (string).
**Optional**: `shell` (bool), `cwd` (path), `env` (map), `timeout_ms` (int), `max_stdout` (bytes cap).

```yaml
- name: build
  exec:
    command: cargo build --release
    cwd: ./crates/engine
    timeout_ms: 300000
    env:
      RUSTFLAGS: "-C target-cpu=native"
```

**Invariants** (from `nika-kernel::process::ShellExecutor`):

- `Command::new` MUST set `kill_on_drop(true)` — no leaked child processes (INV-011).
- Concurrent pipe reading uses `tokio::try_join!` (INV-012).
- Non-zero exit is a task failure by default. Sanitize untrusted
  interpolation through the `shell_quote` filter before the command
  reaches L1.

Cross-link: [`ShellExecutor` trait](/architecture/kernel#shell-executor) (L0.5 kernel contract).

## `fetch` — HTTP request

Issues an HTTP request (GET/POST/etc.), returns bytes or parsed JSON.

**Required**: `url` (string).
**Optional**: `method` (string, default `GET`), `headers` (map), `body`
(string), `json` (any), `timeout_ms` (int), `follow_redirects` (bool),
`response` (hint: `text` | `json` | `bytes`), `extract` + `selector`
(CSS/JSONPath extraction), `session` (bool), `cache` (bool).

```yaml
- name: pull_readme
  fetch:
    url: https://api.github.com/repos/supernovae-st/nika/readme
    method: GET
    headers:
      Accept: application/vnd.github.raw
      Authorization: "Bearer {{ env.GITHUB_TOKEN }}"
    timeout_ms: 30000
```

**Invariants** (from `nika-kernel::http::HttpClient`):

- Output is **tainted** by default (external input is untrusted, T3:A).
- Transient statuses (429, 5xx) are retried per `retry:` on the task.
- Authentication headers never appear in events — they pass through
  `SecretResolver` + `AuditSink::SecretRedacted`.

## `invoke` — call an MCP tool or resource

Calls a tool or reads a resource exposed by a connected **Model Context
Protocol** server. Aliases are declared in the workflow's `mcp:` block.

**Required**: exactly one of `tool` (string) or `resource` (URI string).
**Optional**: `params` (JSON), `mcp` (server alias), `timeout_ms` (int).

```yaml
mcp:
  chroma:
    command: chroma-mcp
    args: ["--collection", "nika_docs"]

tasks:
  - name: search_vectordb
    invoke:
      mcp: chroma
      tool: query
      params:
        query: "{{ tasks.summarize.output }}"
        top_k: 5
```

Routing goes through `ToolExecutor` + `ToolExecute::execute(ToolCall)`
at L0.5 — the same trait the `agent` verb uses to dispatch in-loop
tool calls (`crates/nika-kernel/src/runtime/tool_executor.rs`).

## `infer` — LLM inference

Single-shot call to a language model. One `InferRequest` shape covers
all {STATUS.providers} providers in the catalog.

**Required**: `prompt` (string).
**Optional**: `system` (string), `temperature` (0.0–2.0), `max_tokens`
(u32), `extended_thinking` (bool), `thinking_budget` (u32), `content`
(multi-modal parts), `response_format` (hint), `provider` + `model`
(overrides of workflow defaults).

```yaml
- name: summarize
  infer:
    provider: anthropic
    model: <model-id>                    # see /reference/providers-catalog
    system: "You are a technical writer."
    prompt: |
      Summarize this in one sentence:
      {{ tasks.pull_readme.output }}
    temperature: 0.3
    max_tokens: 500
```

<Note>
  Model identifiers are catalog-validated at parse time. See the
  [Providers catalog](/reference/providers-catalog) for live IDs — we
  never hard-code them in concept docs because model names drift faster
  than the docs refresh cycle.
</Note>

**Invariants** (from `nika-kernel::provider::Provider`):

- Unified `InferRequest` (INV-017) — one DTO, 7 API dialects below.
- Capability rules ([{STATUS.capabilityRules} rules](/reference/capabilities))
  run **before** dispatch. Sending `tools` to a model whose capability
  rule says `tool_calling = false` fails with NIKA-13x before the
  network round-trip.
- Every provider emits the same `InferEvent` stream — `Delta`,
  `ToolUseStart`, `ToolUseDelta`, `Thinking`, `Usage`, `Done`.

Cross-link: [Providers](/concepts/providers) · [`Provider` trait](/architecture/kernel#provider) · ADR-035 (telemetry seams).

## `agent` — multi-turn loop with tools

A model-driven loop: the agent calls tools, observes results, iterates
until a completion signal fires or a budget exhausts.

**Required**: `prompt` (string).
**Optional**: `tools` (list), `max_turns` (u32), `max_tokens` (u32),
`from` (agent preset reference), `skills` (list), `provider` + `model`,
`mcp` (list of server aliases), `temperature`, `token_budget` (u32),
`system` (string), `extended_thinking` (bool), `thinking_budget` (u32),
`depth_limit` (u32), `tool_choice` (string), `stop_sequences` (list),
`scope` (string).

```yaml
- name: investigate
  agent:
    provider: anthropic
    model: <model-id>
    system: |
      You are investigating a GitHub issue. Use tools to gather
      context before answering.
    tools:
      - fetch
      - invoke
    max_turns: 10
    token_budget: 100000
    prompt: |
      Investigate issue #{{ inputs.issue_number }} and summarise
      the root cause in under 200 words.
```

**Invariants** (from `nika-kernel::runtime::agent`):

- `AgentLoopConfig` bounds the loop: `max_turns`, `token_budget`,
  `depth_limit`. The first ceiling hit stops the loop (`AgentStopReason`).
- Every tool call is **taint-checked** — untrusted data cannot reach
  dangerous verbs without an explicit sanitizer.
- The iteration trace (`ToolCallRecord` + `CheckpointMessage`) is
  emitted as events for audit and, optionally, checkpoint resume.

Cross-link: [`AgentLoopConfig`](/architecture/kernel#agent-loop) · ADR-016 (cancellation model).

## Verb selection is exhaustive

The `nika-schema` parser enforces "exactly one verb key per task" at
compile time — the `Verb` enum in
`crates/nika-schema/src/parser/tasks.rs` has five variants, and the
action builder has exactly five match arms. Adding a sixth verb
requires:

1. Extending `RawAction` + the `Verb` enum.
2. Wiring a new action builder arm.
3. Bumping `SchemaVersion` (currently `v1`).
4. Running all 12 admission gates on the consuming L2 verb crate.

This is the single hardest number in the Nika grammar to change. We'd
rather say no.

<Info>
  **Implementation status.** Current verb executors ship as design-only
  kernel traits; the L2 verb crates (`nika-verb-exec`, `nika-verb-fetch`,
  `nika-verb-invoke`, `nika-verb-infer`, `nika-verb-agent`) are admitted
  in Rounds 5–8 (see [Constellation](/reference/constellation)). The
  verb grammar is locked. Implementation tracked via `nika-schema` crate
  (Round 4 admission).
</Info>

## Read next

<CardGroup cols={2}>
  <Card title="Workflows" icon="diagram-project" href="/concepts/workflows">
    How verbs compose into a DAG.
  </Card>
  <Card title="Bindings" icon="link" href="/concepts/bindings">
    How values flow between tasks.
  </Card>
  <Card title="Providers" icon="server" href="/concepts/providers">
    The {STATUS.providers}-provider catalog behind `infer` and `agent`.
  </Card>
  <Card title="Events" icon="signal" href="/concepts/events">
    Every verb emits typed events.
  </Card>
</CardGroup>
