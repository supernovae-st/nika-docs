...
The native argv starter prints a literal message with the system `printf`.
It needs no Cargo project or Rust toolchain:

```bash
nika try snippets/run
```
...
run `nika try 03-exec-pipeline`. That lesson uses explicit sample commands:
...
build or deployment. Read the
[complete pipeline](https://github.com/supernovae-st/nika-spec/blob/main/examples/03-exec-pipeline.nika.yaml).
