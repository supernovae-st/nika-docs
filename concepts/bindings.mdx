...
at runtime against a typed scope (the four value authorities, task-scope
...
<Info>
  **Two expression layers, locked by the spec:** [CEL](https://cel.dev)
  ...
  below is the frozen v1 contract.
</Info>
...
Every binding resolves against one of five named namespaces. The resolver
...
|---|---|---|
| `inputs.*` | Typed workflow inputs, declared in the envelope `inputs:` block — supplied by the caller at launch | `inputs.target` |
...
authorities. A read outside the family (`${{ params.X }}` and friends)
refuses `NIKA-VALUES-003`.

Pick by the role the value plays: a typed parameter the caller supplies is
an `inputs:` declaration, a deployment knob is the same declaration with
`required: false` and a `default:`, a value the file owns is a `const:`
entry, a credential is a `secrets:` reference.
...
`after:` keys (the entry names the producer) · `on_error.recover:` (a
fallback reads a settled record) · an `unwind` task's body (which reads the
**producer it unwinds**, and nothing else) · workflow `outputs:` (the run's
exports read the settled world).
...
three value authorities `inputs` · `const` · `secrets` · the loop locals):
every cross-task dependency is visible at the boundary, named, and typed by
its edge role.
...
tasks:
  loop:
    for_each:
      items: "${{ inputs.count }}"      # fails — the collection needs a list
    infer:
...
- `inputs.*` → tainted (caller-supplied at launch, from outside the file).
