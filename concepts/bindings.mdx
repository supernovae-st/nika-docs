...
The task body is a **pure function of its declared inputs** (`with` ·
`vars` · `env` · `secrets` · the loop locals): every cross-task dependency
is visible at the boundary, named, and typed by its edge role.
