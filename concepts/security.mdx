...
to let that happen silently: there are exactly two doors, both authored, both
requiring a written justification, both recorded in the run receipt.

| Door | Lifts | Shape |
|---|---|---|
| `declassify:` | the **permit-parameterization taint** — an untrusted binding used where a permit is matched | a list; each entry names `from:` (one dotted binding), `to: trusted`, and a non-empty `because:` |
| `inert:` | the **data-as-code sink** — a `nika:fetch` whose resolved URL names a code-bearing class (`NIKA-SEC-008`) | a non-empty string; the string IS the justification |
...
Each door lifts **one** law and nothing else. `declassify:` does not widen
...
`inert:` does not touch the `net.http` boundary or the SSRF floor.

Because both are plain YAML keys, `grep -rn 'declassify:\|inert:'` over a
repo returns every place a law was waived, with the reason attached. That is
the point: the escape hatch is auditable by construction.
