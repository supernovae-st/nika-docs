...
description: "Absent permits is zero authority. SSRF, the shell blocklist, taint, and typed NIKA-SEC errors on the declared boundary."
...
permits:
  fs:
    read: ["./data/**"]        # paths tasks may read
    write: ["./out/**"]        # paths tasks may write
  net:
    http: ["api.github.com"]   # hosts nika:fetch may reach
  exec: ["git", "wc"]          # programs exec: may run (false · true · allowlist)
  tools: ["nika:*"]            # tool ids invoke:/agents may call
```

Two laws govern it:

1. **No `permits:` block → zero authority**, not an unrestricted floor.
   Hardened defaults (SSRF, shell blocklist, secret masking) still apply,
   and they do **not** grant exec, net, fs, or tools. Measured on
   published 0.120.0 and GitHub 0.120.1: `exec: ["printf", "hello"]`
   with no `permits:` is `NIKA-AUTH-006` at check, exit 2
   (`coordination/docs-concept-audit/absent-permits*.json`). Spec
   01-envelope · LAW-AUTH-0324.
2. **Any `permits:` block → default-deny for the whole file.** Anything
   not listed is refused. A task reaching beyond the boundary is caught
   by `nika check` (`NIKA-SEC-004` or `NIKA-AUTH-006`, with the exact
   fix) and again at run time.

<Tip>
  Predict: a file with one `exec:` and no `permits:` — does it run?
  It does not. `nika check --infer-permits` prints the tightest block
  to paste. Infer-only `mock/echo` with `permits: {}` is the declared
  zero, and that is enough for a first run.
</Tip>

You never have to write it by hand: `nika check --infer-permits`
...
<Steps>
  <Step title="Static checks (pure)">
    ...
    (`localhost`, cloud metadata), literal-IP range checks: implicit
    loopback, RFC 1918, link-local/metadata (`169.254.169.254`), CGN,
    IPv6 local ranges, v4-mapped v6. An **exact** host you listed under
    `permits.net.http` (for example `127.0.0.1`) is the spec's grant
    exception — not "every loopback address is always blocked", and not
    "localhost always works". Unlisted aliases stay refused.
  </Step>
...
```yaml
# ❌ tainted output interpolated into the argv
bad:
  with:
    comment: ${{ tasks.fetch_comment.output }}
  exec:
    command: ["echo", "${{ with.comment }}"]
...
Some legitimate workflows do need to cross one of these lines. Nika refuses
to let that happen silently: there is exactly one door, `lift:`, authored,
naming the law it opens, requiring a written justification, recorded in the
run receipt.

| `lift:` entry | Lifts | Shape |
|---|---|---|
| `law: taint` | the **permit-parameterization taint** — an untrusted binding used where a permit is matched | `from:` (one dotted binding) + a non-empty `because:` |
| `law: data-as-code` | the **data-as-code sink** — a `nika:fetch` whose resolved URL names a code-bearing class (`NIKA-SEC-008`) | a non-empty `because:` · no `from:` |
...
load_inventory:
  ...
  lift:
    - law: taint
      from: inputs.p
      because: "deployment-controlled inventory path, reviewed at release"
```

Each entry lifts **one** law and nothing else. `law: taint` does not widen
...
Do not freeze the `NIKA-SEC` family at three codes. The catalog on
[Error codes](/reference/error-codes) is the live set (`NIKA-SEC-001`
