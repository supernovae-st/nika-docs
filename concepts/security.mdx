...
1. **No `permits:` block → the engine floor only.** The hardened
   defaults below (SSRF defense, shell blocklist, secret masking)
...
2. **Any `permits:` block → default-deny for the whole file.** Anything
   ...
   by `nika check`, statically — `NIKA-SEC-004`, with the exact fix —
   and the same boundary is enforced again at run time.
...
<video autoPlay muted loop playsInline poster="/images/posters/permits-audit.png" style={{ borderRadius: "0.75rem" }}>
  <source src="/videos/permits-audit.webm" type="video/webm" />
  <source src="/videos/permits-audit.mp4" type="video/mp4" />
</video>
...
to let that happen silently: there is exactly **one** construct that opens a
law, it is authored, it carries a written justification, and it lands in the
run receipt and the check certificate.
...
`law:` is a closed enum. Twenty-four error-bearing laws exist and **two have
a door**:

| `law:` | What it opens | Shape |
|---|---|---|
| `taint` | the **permit-parameterization taint** — an untrusted binding used where a permit is matched | `from:` names one dotted binding · `because:` non-empty |
| `data-as-code` | the **data-as-code sink** — a `nika:fetch` whose resolved URL names a code-bearing class (`NIKA-SEC-008`) | `because:` non-empty · `from:` is forbidden here |

A lift moves exactly **one** law and nothing else. It never widens
`permits:` — the value is still matched against the declared boundary, so it
can never become a permit bypass — never touches the `net.http` boundary,
never lowers the SSRF floor, and there is no implicit declassification.
A lift whose named law would not have fired on that task is refused with
`NIKA-AUTH-011`, never accepted as a silent no-op: a trapdoor whose whole
value is being countable must refuse to be written when it counts nothing.

Because it is a plain YAML key, `grep -rn 'lift:'` over a repo returns every
place a law was opened, with the reason attached. That is the point: the
escape hatch is auditable by construction.
