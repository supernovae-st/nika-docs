---
title: "Architecture"
description: "Nika is a single Rust binary composed of layered crates, six layers deep plus the L0.5 trait half-layer. Strict downward dependencies, enforced by cargo-deny."
...
You install one `nika` binary (or `@supernovae-st/nika` for apps). This
page is why that binary can `check` a file before it runs: internally it
is **{STATUS.cratesAdmitted} admitted crates** across six layers
(L0 → L5) plus the **L0.5** trait half-layer. Each crate has one job.
Together they parse `.nika`, resolve templates, route to providers,
execute verbs, and emit a structured event stream. Workflow authors do
not pick a layer.
...
`nika-schema` (parser · analyzer · static check) is admitted and live: it is what `nika check` runs. The six cards above are a sample, not the full admitted set.
...
<Steps>
  ...
  <Step title="CANARY → PARITY → REVIEW → COMMIT">
    End-to-end `.nika` canary. Golden diff vs legacy (Phase 5+).
