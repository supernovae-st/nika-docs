...
chat or piped on stdin, where the `.nika` suffix is gone.
...
**`unwind`** — a cleanup edge on a producer that **started**. Spec 03-dag:
the unwind task runs after that producer, success or failure. A producer
that never started has no cleanup. A terminal report task (`after: {x:
terminal}`) is a different edge — it is not unwind.
→ [Workflows](/concepts/workflows)
