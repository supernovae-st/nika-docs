...
Here is a **historical** capture (2026-07, two-task `exec` workflow). The
...
Two commands inspect a recorded run. With no path they read the
workspace's latest journal:

```bash
nika trace replay   # re-render the run, live
nika trace show     # print the final card only
```
...
- **REPLAYED** — you passed a separately supplied fresh journal; verify
  compares it. The command **never starts** that second run.
