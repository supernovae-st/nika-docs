...
What is a `.nika` file, and what happens when you run it?
...
```bash
nika check hello.nika   # clean
nika run hello.nika --max-cost-usd 0
# said "mock(echo) · Say hi in one line."
```

Swap `model:` later for a local or cloud seat. The file is the contract;
`--var` / SDK `inputs` supply caller values.

<Info>
  Envelope law: [nika-spec 01-envelope](https://github.com/supernovae-st/nika-spec/blob/main/spec/01-envelope.md).
  [Live binary](/reference/status) is the implementation you download.
</Info>
...
- **It satisfies `required: true`.** A required input with no default must
  be supplied at launch. Measured on 0.120.1: `nika check` stays clean and
  prints `HINT [inputs] required input(s) with no default · pass at run
  time: --var name=…`. `nika run` refuses before any task:
  `NIKA-1708 · missing required inputs` (exit 3). The SDK throws
  `NikaOperationError` with that code. This is admission, not a
  task-level `NIKA-VAR-001`.
