...
Nika ships with **{STATUS.providers} LLM providers** in its catalog,
orchestrated behind one unified `InferRequest`. The `infer:` verb takes
a `provider` + `model` pair, looks up capability rules
...
pub struct InferRequest {
    ...
    pub model: ModelId,                // e.g., "claude-sonnet-4-20250514"
...
tasks:
  - id: first
    infer:
      provider: anthropic                    # ← change to openai, gemini, groq, ...
      model: claude-sonnet-4-20250514        # ← change to matching model
...
<Warning>
  ...
  Lands with `nika-verb-infer` admission (Round 8).
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Concepts · Verbs" icon="play" href="/concepts/verbs">
    The `infer:` verb and its four siblings (exec / fetch / invoke / agent).
  </Card>
