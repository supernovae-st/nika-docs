---
title: "Events"
description: "Five observability channels. Every run emits typed events, metrics, traces, billing records, and append-only audit entries."
sidebarTitle: "Events"
icon: "signal"
---
...
<Info>
  **Target design banner.** The `EventSink`, `AuditSink`, `BillingSink`,
  `MetricsExporter`, and `TracerProvider` traits are all defined today
  in `nika-kernel` (L0.5). End-to-end emission from admitted L2 verb
  crates is **target design — tracked via Rounds 5–9 of verb crate
  admissions**. The event shapes below are stable; the per-verb emission
  sites mature across the constellation.
</Info>
...
- `workflow.skipped` — top-level `condition:` evaluated to false.

### Task-level

- `task.started` — a task begins executing.
- `task.completed` — a task ends successfully.
- `task.failed` — a task errored out; payload carries the NikaCode.
- `task.skipped` — task-level `condition:` evaluated to false.
...
Cross-link: OTel GenAI spec at <https://opentelemetry.io/docs/specs/semconv/gen-ai/>.
...
Both land as additive, `#[non_exhaustive]` extensions. At v0.100 they
...
```bash
nika run workflow.nika.yaml --events ndjson               # stdout
nika run workflow.nika.yaml --events ndjson > run.log     # file
nika run workflow.nika.yaml --events otlp://localhost:4317 # OTLP gRPC
```

The `nika-serve` L4 interface crate admits in Round 11 and exposes
NDJSON / OTLP / SSE over HTTP. The CLI flag is target design; today
`EventSink` only has a mock implementation in `nika-kernel-mock`.
...
<CardGroup cols={2}>
  <Card title="NDJSON reference" icon="file-code" href="/reference/ndjson">
    Full event kind catalog with per-kind payload schemas.
  </Card>
