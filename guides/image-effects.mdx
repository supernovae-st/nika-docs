...
effects and get an artifact that is **byte-identical** for identical
`(input, args)` on the **same pinned engine** — not a promise across
engine versions.
...
```yaml fragment · paste into the file above
  variants:
    with:
      src: ${{ tasks.generate.output.images[0].path }}
    for_each:
      items: ["bw", "gameboy", "cga"]
    invoke:
      tool: "nika:image_fx"
      args:
        input: "${{ with.src }}"
