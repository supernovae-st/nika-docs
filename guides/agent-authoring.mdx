...
<Steps>
  ...
  <Step title="Repair: the error names its fix">
    ...
    | `NIKA-PARSE-024` | `depends_on:` is dead: data → a `with:` binding · control → `after: { x: succeeded }` (`nika check --fix` migrates) |
    | `NIKA-DAG-001` | cycle: remove the back-edge |
    | `NIKA-DAG-005` | unknown `after:` predicate: the set is closed — `succeeded` · `failed` · `skipped` · `terminal` |
...
4. **`when:` is a `${{ }}` CEL boolean or the literal `true`/`false`** ·
   ...
   only (`vars` · `env` · `with` · `item`/`index`), post-gate; the
   always-pattern is `after: { x: terminal }`, never a `when:` trick.
