...
<Steps>
  ...
  <Step title="Repair: the error names its fix">
    ...
    | `NIKA-VAR-001` | undeclared name: declare it under the authority its role commands — `inputs:` · `config:` · `const:` · `secrets:` — or fix the typo |
...
4. **`when:` is a `${{ }}` CEL boolean or the literal `true`/`false`** ·
   ...
   only (the value authorities `inputs` · `config` · `const` · `secrets`,
...
10. **Every value lands under one of four authorities** · `inputs:`
    (the caller supplies it) · `config:` (the deployment supplies it) ·
    `const:` (the file owns it) · `secrets:` (a governed store). `vars:`
    and `env:` are dead envelope fields (`NIKA-VALUES-001` /
    `NIKA-VALUES-002`) — classify by the role the value plays, never
    rename in bulk.
