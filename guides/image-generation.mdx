...
## Why the save is gated

Image bytes land on disk under `permits.fs.write`, the engine verifies
returned MIME types, a provenance manifest records the resolved request,
and result URLs are never fetched (that would reopen SSRF). Local-first
is a first-class provider. The PNG can carry a `nika` tEXt chunk — the
same informal provenance practice ComfyUI and InvokeAI use. The argument
contract is in [builtins](/reference/builtins#media-4) and
`nika catalog --tools`.
