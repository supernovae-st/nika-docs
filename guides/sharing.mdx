...
Nika workflows are one portable `.nika` file, so sharing one is just
sharing text. [**nika-registry**](https://github.com/supernovae-st/nika-registry)
adds a trust layer: every entry is re-proven by CI — pinned to a full
commit + sha256, re-run through the conformance oracle, and certified by
the engine's static analysis. You see what a workflow *can do* — exec,
tools, cost ceiling, secrets — **before a single token is spent**.
