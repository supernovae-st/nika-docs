...
permits:                            # the blast radius · default-deny once present
  exec: false                       # no program launches from this shape
  tools: ["nika:read", "nika:write"]
  fs:
    read: ["./README.md"]           # SLOT: same path as const.source · move one, move both
    write: ["./output.md"]          # SLOT: same path as the persist destination
...
config:
  threshold:                        # SLOT: the trigger condition value · a dial, not the subject
    type: integer
    default: 100
...
permits:
  ...
  net:
    http:
      - "api.example.com"           # SLOT: the watched host · same as const.source_url
      - "hooks.slack.com"           # SLOT: the webhook host · nothing else may leave
...
tasks:
  ...
  act:
    ...
    when: ${{ with.value > config.threshold }}   # SLOT: the CEL condition
    invoke:
...
permits:
  ...
  fs:
    read: ["./items/**"]            # SLOT: the tree the glob walks · same as const.collection_source
...
permits:
  ...
  net:
    http: ["api.example.com"]       # SLOT: the data-source host · same as const.source_url
  fs:
    read: ["./state/etl-state.json"]    # SLOT: the cursor file · same as const.state_path
    write: ["./state/etl-state.json"]   # the same file, read AND written — that is why it is const
...
permits:
  exec: false
  tools: ["nika:assert", "nika:done", "nika:read"]   # the agent's grant + the confirm gate
...
permits:
  ...
  net:
    http: ["example.com"]           # SLOT: the crawled host · same as const.site_url
  fs:
    write: ["./out/brief.json"]     # SLOT: same path as const.out_path
...
inputs:
  subject:                          # SLOT: what the asset is about · the caller's subject
    type: string
    default: "a calm cosmic landing hero"   # a default keeps the skeleton green with zero args
...
permits:
  ...
  fs:
    write: ["./out/assets/**"]      # SLOT: the asset tree · same as const.out_dir
  ...
  brief:
    infer:
      ...
      prompt: |
        ...
        Write one vivid, concrete image prompt for: ${{ inputs.subject }}.
...
permits:
  ...
  tools: ["nika:fetch"]
  net:
    http: ["api.example.com"]       # SLOT: the product API host · same as const.api_base
