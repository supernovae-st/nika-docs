...
<AccordionGroup>
  <Accordion title="Can I run Nika today?" icon="play">
    Not yet — the engine is mid-rebuild (Diamond). The **first usable
    release is v0.81** (language v0.1 + a vertical slice you can launch),
    summer 2026. What works today: the JSON schema validates your
    `.nika.yaml` in any editor ([editors guide](/getting-started/editors)),
    and every example in these docs is conformance-validated YAML you can
    write against now.
  </Accordion>
  <Accordion title="Why does the language say v1 but the engine v0.x?" icon="code-branch">
    Two independent axes. The **language envelope `nika: v1` is frozen
    forever** — files you write never break. The **engine** versions
    honestly (`v0.81`, `v0.82`, …, forever-v0.x — no fake-stable v1.0).
