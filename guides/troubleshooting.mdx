...
| « unknown binding » | `NIKA-VAR` · `NIKA-VALUES` | Six namespaces only — the four value authorities (`inputs` · `config` · `const` · `secrets`) plus `with` and `tasks`. Typos like `tsaks.` are caught at validation; a read of the dead `vars`/`env` namespaces is `NIKA-VALUES-001`/`-002`, and any other value namespace is `NIKA-VALUES-003`. |
