...
machines later. After the run above, the journal is already on disk
(the `trace:` line). With no path, these commands read that latest
journal:
...
journal path** — there is no workspace-latest default. The path is the
`trace:` line the run card already printed (During, above), or a row
from `nika trace ls`. A later run prints a new timestamped name; do not
invent one.

```text
nika trace ls
nika trace export <path-from-the-run-card-or-ls>
# default sidecar: same stem, `.otlp.jsonl` beside the journal
nika trace export <path-from-the-run-card-or-ls> -o run.otlp.jsonl
```
