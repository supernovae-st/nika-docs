...
```yaml standup-digest.nika
# Pull commits and tickets in parallel, then digest.
nika: standup-digest

model: mock/echo                       # simulation · swap for ollama/qwen3.5:4b or any provider
...
```text
$ nika inspect standup-digest.nika
...
$ nika run standup-digest.nika
  ...
  (historical capture 2026-07-28 · re-run for a current journal)
```
...
machines later. After a run, the journal is already on disk. With no
path, these commands read the workspace's latest journal:

```bash
nika trace show     # storyboard + waterfall + verdict card — a reading
nika trace replay   # re-render the run live · never re-execute — a reading
nika trace verify   # walk the hash chain — the proof, not a reading
```
...
happened. Neither `show` nor `replay` proves the chain;
`nika trace verify` does ([Traces & replay](/concepts/traces)).
...
Unlike `show` / `replay` / `verify`, **`nika trace export` requires the
journal path** — there is no workspace-latest default. Fill `<path>`
with a row from `nika trace ls`.

```text
nika trace ls
nika trace export <path>
# default sidecar: same stem, `.otlp.jsonl` beside the journal
nika trace export <path> -o run.otlp.jsonl
```
...
From an app, iterate `run.events()` then await `run.result()` — see
[SDK runtime events](/sdk/runtime/events). `traceVerify` only reads
evidence the run already left.
