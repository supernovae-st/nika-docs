...
That's it. In any workflow (the same local default the engine scaffold
and brew caveat teach):
...
<Tip>
  `qwen3.5:4b` is the first-run local default: small enough for most
  laptops, good enough for structured extraction. Swap the tag for
  anything in the [Ollama library](https://ollama.com/library):
  `llama3.2`, `mistral`, `phi4`, larger Qwen variants. Rehearse with
  `model: mock/echo` when you want zero daemon and zero keys — mock
  simulates the model reply; it does not disable tools or effects.
</Tip>
...
```yaml
model: ollama/qwen3.5:4b         # local · unpriced · private
# model: openai/gpt-4.1-mini     # cloud · pin a cataloged id
# model: xai/grok-4
# model: huggingface/…           # Hub-served
# model: nvidia/…                # NIM
```
