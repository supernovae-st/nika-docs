...
<Info>
  **Canonical source:** [`RUST_ENFORCEMENT.md`](https://github.com/supernovae-st/nika/blob/main/RUST_ENFORCEMENT.md) §12 Gates · checker `scripts/ci/check-crate-gates.sh` (machine-readable JSON output).

  **Live progress:** {STATUS.cratesAdmitted} crates admitted · {STATUS.wipCrates.length} WIP ({STATUS.wipCrates.length ? STATUS.wipCrates.join(", ") : "none"}) · projected horizon {STATUS.cratesTarget} (cap {STATUS.cratesCap}).
</Info>
...
<Steps>
  ...
  <Step title="Gate 9: CANARY E2E">
    A `.nika` canary workflow under `tests/canary-<crate>.nika`
...
The engine-dev rhythm during the Diamond rewrite was **1 crate admission =
1 commit**, with per-admission alpha tags on the 0.8x train. That cadence
is historical — public releases are tagged engine versions (today
{STATUS.version}), not per-crate alphas.
