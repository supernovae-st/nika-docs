...
description: "Q1-Q13: the 13 architectural decisions that lock the L0 + L0.5 layer shape."
...
These are **engine-internal** locks, not workflow-language rules.
Thirteen questions, thirteen locks. They shape the **L0 foundation**
({STATUS.cratesAdmitted} admitted crates in the live snapshot) and the
**L0.5 kernel contract**. Revised twice by swarm audit: current state is
rev. 3. Do not read "Round 4" as unfinished product work — `nika check`
already runs on admitted schema.
