...
<AccordionGroup>
  ...
  <Accordion title="FCI-003: Single version marker per on-disk artifact" icon="file-code">
    ...
    additions are **additive** and never change it. There is no `nika: v2` —
    ever: pre-1.0 grammar changes happen inside `v1` (the pre-1.0 stability
    contract) · after engine 1.0.0 the grammar is additive only.
