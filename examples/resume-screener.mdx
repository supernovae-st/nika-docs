...
permits:                    # the file IS the blast radius · no net category at all:
  ...
  exec: false               # no shells either · four tools and one local model
...
inputs:
  role:
    ...
    description: "The role every CV is scored against"

config:
  shortlist_size:
    type: integer
    default: 5
    description: "How many candidates reach the brief"

const:
  cv_glob: "./hiring/inbox/*.md"   # the pool the permit above pins · never caller-movable
...
tasks:
  ...
  shortlist:
    ...
    invoke:
      ...
      args:
        input: "${{ with.ranked }}"
        expression: ".[:${{ config.shortlist_size }}]"
