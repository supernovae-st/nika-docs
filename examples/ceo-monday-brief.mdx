  ...
  founders_webhook:
    ...
    egress:                       # sanction the one send · the secret IS the URL
      - to: "nika:notify"
        host_from_self: true
permits:
  tools: ["nika:convert", "nika:date", "nika:fetch", "nika:inspect", "nika:jq", "nika:emit", "nika:notify", "nika:prompt", "nika:read", "nika:write"]
  exec: ["git"]
...
tasks:
  ...
  # ── the human gate · the trifecta judge demands it ──
  # Untrusted content (the fetched news) reaches the write and the ping
  # while the KPI sheet grants a private read: the lethal trifecta. The
  # brief therefore lands on a human first — nika:prompt blocks until
  # answered, and the save fires only on approval (NIKA-SEC-009).
  approve:
    with:
    ...
    invoke:
      ...
      args:
        message: |
          The Monday brief is ready to land:

          ${{ with.brief }}

          Save it and ping the founders? [yes/no]
