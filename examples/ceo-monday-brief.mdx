...
```mermaid
flowchart LR
  news["news · fetch jq"]:::invoke --> brief["brief · thinking"]:::infer
  pulse["pulse · git shortlog"]:::exec --> brief
  kpi_raw["kpi_raw · read"]:::invoke --> kpis["kpis · convert"]:::invoke
  kpis --> revenue["revenue · jq add"]:::invoke
  kpis --> brief
  revenue --> brief
  brief --> save["save · dated path"]:::invoke
  stamp["stamp · nika:date"]:::invoke --> save
  save --> bill["bill · inspect cost"]:::invoke
  bill --> ping["ping · notify + cost"]:::invoke
  stamp --> ping
