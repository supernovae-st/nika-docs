...
config:
  window:
    type: timestamp
    default: "2026-07-28T09:00:00Z"
    description: "The absolute departure window the train holds for"
...
permits:
  exec: ["./scripts/release.sh", "cargo"]   # the three gates + the one ship script
  net:
    http: ["api.example.com", "hooks.example.com"]   # prod's version endpoint + the webhook
  tools: ["nika:date", "nika:assert", "nika:prompt", "nika:wait", "nika:fetch", "nika:emit", "nika:notify"]
...
tasks:
  ...
  conductor:
    ...
    invoke:
      tool: "nika:prompt"
      args:
        message: "Gates GREEN in ${{ with.gate_time }} min. Ship ${{ inputs.version }} in the ${{ config.window }} window?"
        default: false
