...
tasks:
  ...
  t1:
    after:
      gates_green: succeeded           # state, no data · the clock stops on a green board
    invoke:
      tool: "nika:date"
      args: { op: now }
    ...
    invoke:
      ...
      args:
        message: |
          ...
          Ship ${{ vars.version }} · departure window opens in ${{ vars.hold_for }}?
          (depart armed: ${{ vars.depart }})
  ...
  ship:
    ...
    exec:
      # argv, never a shell string: `${{ vars.version }}` is substituted as ONE
  ...
  verify:
    after:
      ship: succeeded                  # never probe prod for a release that did not go out
    invoke:
      tool: "nika:fetch"
      args:
