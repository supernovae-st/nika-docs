...
```mermaid
flowchart LR
  live["live · fetch + retry"]:::invoke --> drift["drift · json_diff"]:::invoke
  baseline["baseline · nika:read"]:::invoke --> expected["expected · merge_patch"]:::invoke
  expected --> drift
  live --> fingerprint["fingerprint · blake3"]:::invoke
  drift -->|"when size &gt; 0"| explain["explain"]:::infer
  explain --> alert["alert · nika:notify"]:::invoke
  fingerprint --> alert
  drift --> record["record · nika:emit"]:::invoke
  fingerprint --> record
