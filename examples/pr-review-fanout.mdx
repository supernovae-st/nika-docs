...
{/* showcase:dag-begin pr-review-fanout.nika */}
```mermaid
flowchart TD
  ...
  approve --> changed
  approve -.->|success| changed
  changed --> files
  approve --> todo_sweep
  approve -.->|success| todo_sweep
  files --> reviews
  reviews --> merged
  changed --> summary
  merged --> summary
...
Watch the graph execute: the human gate first, independent branches in
parallel behind it, the agent fan-out mid-graph, and the join that waits
for both branches before `summary`:
...
{/* showcase:begin pr-review-fanout.nika */}
```yaml pr-review-fanout.nika
nika: pr-review-fanout
...
tasks:
  ...
  changed:
    ...
    on_error:
      # Not an empty diff: the notice carries no `src/` line, so the swarm
      # has nothing to review, and `summary` repeats it instead of a verdict
      # earned on files nobody read.
      recover: "diff unavailable: git failed or was refused. No changed files were collected."
  ...
  summary:
    ...
    infer:
      ...
      prompt: |
        Changed files, or the notice that the diff was unavailable ·
        ${{ with.changed }}
        Merge these review findings into REVIEW.md · blockers first ·
        ${{ with.merged }}
        Open TODO debt found by grep ·
        ${{ with.todo_sweep }}
        End with a verdict · ship / fix-first / redesign. If the diff was
        unavailable, say so and invent no findings.
...
outputs:
  diff:
    value: ${{ tasks.changed.output }}
    description: "The changed-file list, or the explicit notice that Git was refused"
  findings:
