...
{/* showcase:dag-begin meeting-actions.nika */}
```mermaid
flowchart LR
...
{/* showcase:begin meeting-actions.nika */}
```yaml meeting-actions.nika
nika: meeting-actions

# The house seat — a THINKING model: the think block and the JSON both count
# against `max_tokens` (engine#428), so a schema call sizes its ceiling for
# think + answer, never for the answer alone.
...
tasks:
  ...
  extract:
    ...
    infer:
      max_tokens: 3000                 # think block + the actions JSON · a ceiling makes the cost report a number, not UNBOUNDED
      prompt: |
...
<Steps>
  <Step title="A typed input with a default">
    `transcript_path` is `type: string` with a `default:` path — the
    caller may omit it. The engine still validates the value before any
    task runs. There is no `required: true` on this input.
  </Step>
  ...
  <Step title="Write, then a success-only log">
    `save` writes the JSON. `trace` is not a parallel sibling: it runs
    `after: { save: success }` — only if the write succeeded.
  </Step>
</Steps>
