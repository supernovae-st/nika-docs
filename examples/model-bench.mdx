...
{/* showcase:dag-begin model-bench.nika */}
```mermaid
flowchart LR
...
{/* showcase:begin model-bench.nika */}
```yaml model-bench.nika
nika: model-bench
...
```bash
nika try model-bench
```

To run a file you own, copy the YAML on this page to `model-bench.nika`,
pull the seats you want to compare, then:

```bash
ollama pull qwen2.5:14b llama3.2:3b qwen2.5:0.5b
nika check model-bench.nika
nika run model-bench.nika
...
and read the warm numbers. Swap any seat for `openai/…`,
`xai/…` or `anthropic/…` to bench a cloud contender against your
