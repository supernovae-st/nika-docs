...
```mermaid
flowchart LR
  batch["batch · nika:uuid"]:::invoke --> escalate["escalate · nika:notify"]:::invoke
  queue["queue · nika:read"]:::invoke --> triage["triage · schema"]:::infer
  triage --> urgent["urgent · nika:jq"]:::invoke
  urgent -->|"when size &gt; 0"| escalate
  triage --> board["board · nika:write"]:::invoke
  batch --> board
