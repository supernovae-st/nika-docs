...
tasks:
  ...
  plan:
    ...
    infer:
      max_tokens: 600            # planning is short · cap it and the cost report is a ceiling
      prompt: |
        Question · ${{ with.question }}
        Files · ${{ with.files }}
        For each file path, write ONE focus: what this file should be read
        for, given the question. Do not guess contents — assign an angle.
      schema:
  ...
  cards:
    ...
    infer:
      max_tokens: 800            # a card, not an essay
      prompt: |
        Read ONE document and return its card.
        Focus · ${{ item.focus }}
        Document (${{ item.path }}) ·
        ${{ item.text }}
      schema:
