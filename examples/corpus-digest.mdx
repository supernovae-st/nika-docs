...
# The house seat — a THINKING model. Measured: at caps of 600, 2000 and
# 3000 tokens qwen3.5:4b spent the whole budget reasoning before the JSON
# and never closed it (NIKA-INFER-002 · engine#428). So every stage's
# ceiling below is sized for the think block AND the answer — both count
# against `max_tokens`. One resident seat also spares the model-swap
...
tasks:
  ...
  plan:
    ...
    infer:
      max_tokens: 6000           # think block + the form · the cap is margin, not hope
      prompt: |
  ...
  cards:
    ...
    infer:
      max_tokens: 6000           # think block + a card, not an essay
      prompt: |
  ...
  brief:
    ...
    infer:
      max_tokens: 8000           # bounded synthesis on the default local seat
      prompt: |
      ...
      # Extended thinking requires a reasoning-capable seat in the catalog.
      # The default local seat does not declare that capability.
...
| `thinking:` budget on the synthesis | `brief` | [The 4 verbs](/concepts/verbs) |
