...
- Brief the top THREE pages instead of one: bind `top: ${{ tasks.map.top }}` in `with:` and fan out with `for_each: ${{ with.top }}`. The fan-out tier shows how ([Competitor radar](/examples/competitor-radar)).
