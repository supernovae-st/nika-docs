...
```mermaid
flowchart LR
  ledger["ledger · nika:read"]:::invoke --> rows["rows · nika:convert"]:::invoke
  rows --> overdue["overdue · nika:jq"]:::invoke
  overdue -->|"when size &gt; 0"| drafts["drafts"]:::infer
  drafts --> approve["approve · nika:prompt"]:::invoke
  approve -->|"when yes"| save["save · nika:write"]:::invoke
