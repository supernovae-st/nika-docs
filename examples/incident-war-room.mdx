...
description: "T4 epic · SRE — the postmortem drafts itself, after recovery is PROVEN, not assumed."
...
```mermaid
flowchart LR
  logs["logs · journalctl"]:::exec --> timeline["timeline · schema"]:::infer
  status["status_history · fetch + retry"]:::invoke --> timeline
  runbook["runbook · nika:read"]:::invoke --> timeline
  timeline --> settle["settle · nika:wait 60s"]:::invoke
  settle --> recheck["recheck · fetch state"]:::invoke
  recheck --> confirmed["confirmed · nika:assert"]:::invoke
  timeline --> postmortem["postmortem · thinking"]:::infer
  confirmed --> postmortem
  postmortem --> save["save + on_finally"]:::invoke
