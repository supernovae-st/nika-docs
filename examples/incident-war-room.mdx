...
tasks:
  ...
  approve:
    invoke:
      ...
      args:
        message: |
          Incident war-room for ${{ vars.service }} will:
            · read git history for the last ${{ vars.log_window }}
            · poll ${{ vars.status_url }} (twice, ${{ vars.settle }} apart)
            · read ${{ vars.runbook }}
            · write ./incidents/${{ vars.service }}-postmortem.md
            · ping the on-call webhook (armed: ${{ vars.alert }})
  ...
  deploys:
    ...
    when: ${{ with.go == true }}
    exec:
      command: ["git", "--no-pager", "log", "--oneline", "--since=${{ vars.log_window }}"]
    on_error:
  ...
  settle:
    after:
      timeline: succeeded              # state, no data · wait only once there is something to confirm
    invoke:
      tool: "nika:wait"
      args: { duration: "${{ vars.settle }}" }
  ...
  alert:
    ...
    invoke:
      ...
      args:
        ...
        message: "${{ vars.service }} war-room settled · postmortem draft ${{ with.outcome }}"
        severity: warning
