...
tasks:
  ...
  changelog:
    ...
    after:
      copy: succeeded              # state, no data · the anchor is on disk before we edit
    invoke:
      tool: "nika:edit"
      args:
        path: "${{ vars.changelog_out }}"
  ...
  announce:
    with:
      notes_headline: ${{ tasks.notes.output.headline }}
    when: ${{ vars.announce == true }}   # OFF by default · a rehearsal must not ping the team
    after:
      changelog: succeeded
    invoke:
      tool: "nika:notify"
