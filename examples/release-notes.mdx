...
inputs:
  since_tag:
    type: string
    default: "v0.80.0"
    description: "The tag the release range starts from"
...
permits:
  exec: ["git"]                   # the history read · no other program
  net:
    http: ["hooks.example.com"]     # the team webhook host · nothing else leaves
  fs:
    read: ["./CHANGELOG.md"]      # nika:edit reads before it writes
    write: ["./CHANGELOG.md"]     # the ONE file the release touches
  tools: ["nika:edit", "nika:notify"]

tasks:
  history:
    exec:
      command: ["git", "log", "${{ inputs.since_tag }}..HEAD", "--oneline", "--no-merges"]
    ...
    invoke:
      ...
      args:
        ...
        replace: |
          ...
          ## ${{ inputs.since_tag }}..HEAD · ${{ with.notes_headline }}
