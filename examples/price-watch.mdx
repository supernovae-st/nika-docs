...
{/* showcase:dag-begin t1-price-watch.nika.yaml */}
```mermaid
flowchart LR
...
inputs:
  product_api:
    type: uri
    default: "https://api.shop.example.com/v1/products/macbook-air"
    description: "The product endpoint to watch"

config:
  alert_below:
    type: number
    default: 899
    description: "Ping me once the price drops below this"
...
permits:
  exec: false                     # no shells · two tools and one comparison
  net:
    http: ["api.shop.example.com", "hooks.example.com"]   # the shop + the webhook · nothing else leaves
  ...
  alert:
    ...
    invoke:
      ...
      args:
        ...
        message: "Price drop · ${{ with.name }} is now ${{ with.price }} (target ${{ config.alert_below }})"
        severity: info
...
<Steps>
  ...
  <Step title="CEL gates the alert">
    ...
    `when: ${{ with.price < config.alert_below }}` is a plain CEL
