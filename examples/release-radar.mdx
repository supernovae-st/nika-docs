...
inputs:
  releases_feed:
    type: uri
    default: "https://github.com/tokio-rs/tokio/releases.atom"
    description: "The dependency release feed to watch"

const:
  state_path: "./state/release-radar.json"   # the file owns its own cursor · not caller-movable

permits:
  exec: false                     # no shells · five tools and a diff
  net:
    http: ["github.com"]          # the feed host · nothing else leaves
  fs:
    read: ["./state/**"]          # the cursor, read on entry
    write: ["./state/**"]         # the cursor, rewritten on exit
  tools: ["nika:jq", "nika:read", "nika:fetch", "nika:json_diff", "nika:write"]
