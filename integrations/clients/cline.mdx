...
description: "Add the read-only Nika oracle to Cline's cline_mcp_settings.json — hot-reloaded, no restart."
sidebarTitle: "Cline"
icon: "robot"
---
...
`nika wire cline` resolves this for you: a host IDE's existing extension
dir wins (the live, hot-reloaded file); none existing falls back to the
stable home. Idempotent, unrelated servers preserved.

## Verify

Cline's MCP Servers view lists `nika` with the nine `nika_*` tools
