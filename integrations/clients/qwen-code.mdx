...
description: "Add the read-only Nika oracle to Qwen Code's settings.json — same key as Gemini CLI, its own dotdir."
...
`nika wire qwen` writes exactly this — only the `mcpServers` key is
touched, every other setting survives, and re-running prints `· current`
(idempotent).
