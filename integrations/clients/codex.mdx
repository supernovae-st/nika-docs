...
`nika doctor` answers "is my suite coherent?" — it names any kit lagging
or leading the binary's train, with the exact fix per client. Drift is
advisory (warn · exit 0), so it never breaks automation.
