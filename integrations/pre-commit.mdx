...
description: "Audit every .nika before it lands: the check verdict — DAG, types, permits, secret flows, cost floor — at commit time, before a single token is spent."
sidebarTitle: "pre-commit"
icon: "code-commit"
---

The engine ships a [pre-commit](https://pre-commit.com) manifest, so `nika
check` runs as a hook on every staged `.nika`. A workflow that names a
...
repos:
  - repo: https://github.com/supernovae-st/nika
    rev: v0.107.0       # pin a tagged release
    hooks:
      - id: nika-check
```
...
git commit             # every staged .nika is audited from now on
```
