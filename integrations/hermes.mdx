...
- Always pass `--max-cost-usd` on paid cloud models. Two guardrails, two
  strengths: when the workflow file's own `model:` prices above the budget,
  the run **refuses to start** (exit 2, zero tokens) — prefer the in-file
  model for budgeted runs. A `--model` override is not priced by that
  pre-start gate today (0.98.x): the run starts and the budget binds
  mid-run — the crossing call completes, nothing new starts, the run fails
...
- **Trust the log, not the model's word.** The deterministic proof the
  ...
  `MCP server 'nika' (stdio): registered 8 tool(s)`. In-session the tools
