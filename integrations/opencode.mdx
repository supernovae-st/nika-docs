...
Expected tool trail: `nika compile chain --json` (incomplete questions,
preview only) → `--answer` until Ready → write an explicit destination →
edit → `nika check` (static audit, nothing spent) →
`nika run … --model mock/echo` (offline) → receipts in `.nika/traces/`.
...
| a repeated AI pipeline (digest, triage, ETL, report, multi-model bench) | a `.nika` OpenCode authors once |
