...
| **Sovereign local lane** | `nika model serve --model x.gguf` ° — the in-process candle sidecar (loopback · feature `local-infer`) · `pull`/`list`/`rm` acquire and manage GGUFs | [CLI reference](/reference/cli) |

° `serve` needs the `local-infer` build feature — absent from released binaries up to 0.100; `pull`/`list`/`rm` ship in every release.
