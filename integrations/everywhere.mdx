...
<Info>
`nika doctor` is the coherence oracle: it reads what each client
actually loads, names any kit lagging or leading the binary's train,
and prints the exact per-client fix. Drift is advisory (warn · exit 0)
— a diagnosis never breaks automation.
</Info>
