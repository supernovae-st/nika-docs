...
LM Studio follows Cursor's `mcp.json` notation (v0.3.17+). Edit it in-app —
`Program` tab → `Install > Edit mcp.json` (the safe path: on some macOS
installs the file lives under `~/.cache/lm-studio/` instead of the
documented `~/.lmstudio/`) — and add:
...
goose (the AAIF agent) discovers MCP extensions from its config. Add the
oracle as a stdio extension:
...
goose also reads `AGENTS.md` natively — `nika init` in the repo teaches it
the language with zero extra wiring. The binary install is a prerequisite
(brew or a release tarball; goose's directory entry documents the same).
