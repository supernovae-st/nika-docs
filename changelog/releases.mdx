---
title: "Releases"
description: "The full arc — the idea (2025), the exploration era, the Diamond rebuild, and every public release since v0.90.0. One source of truth: `CHANGELOG.md` on the `main` branch."
...
1. **Origins** (2025 → April 2026) — the idea, then a 14-week private
   exploration that grew to a 322k-LOC engine and taught the lesson Diamond
   is built on.
2. **The Diamond rebuild** (April 13, 2026 →) — a ground-up rewrite, one
   crate at a time, 12 gates each.
3. **Public releases** (June 21, 2026 →) — tagged binaries, curated
   changelogs, real semver.
...
<Update label="Diamond begins" description="2026-04-13 · the orphan branch · v0.80.0-alpha.0" tags={["foundation", "milestone"]}>
  **The rewrite starts.** An orphan branch created from scratch (as
  ...
  the exploration era. The constraint that defines everything after:
...
<Update label="The exploration era" description="2026-01-01 → 2026-04-13 · ~4,600 commits · v0.1 → v0.79.3" tags={["origins"]}>
  **The first commit lands on New Year's Day 2026**: `initial nika v0.1
  ...
  fine-grained executor events. Fourteen weeks of full-speed exploration
  (~4,600 commits, peaking at 2,900+ in March) grew it into a working
  322k-LOC, 31-crate engine at v0.79.3.

  It also surfaced the lesson Diamond is built on: at that scale the
  biggest correctness risk stops being algorithmic and becomes
  **context** — no human or AI assistant could hold the system in
  working memory, so refactors happened by pattern-match, and
  pattern-match hallucinates. The exploration era is preserved as a
  read-only private archive (`brouillon`); nothing ships from it. Diamond
  reads it, then rewrites from scratch.
</Update>

<Update label="The idea" description="2025 · before any code" tags={["origins"]}>
  Nika started as a frustration, in 2025, before a single line existed:
  useful AI work kept disappearing into chat histories. A great run — a
  research brief, a review procedure, a data pipeline — lived and died
  inside one conversation, unrepeatable and unownable. The bet:
  **durable agent work should be source code** — one file you can run,
  diff, review, and keep; local-first; any model; sovereign by default.
  The first exploratory commit followed on January 1, 2026.
</Update>
...
<CardGroup cols={2}>
  ...
  <Card title="Roadmap" icon="map" href="/changelog/roadmap">
    What's next: the 0.9x hardening train, the 1.0.0 public API lock,
    1.x minors completing the 42-crate foundation, then the 2.0
    Connectome era and the ecosystem tier.
  </Card>
</CardGroup>
