...
<Update label="v0.117.1" description="2026-09-02 · what a public gauntlet found, closed · one provider count · the golden test grows cases" tags={["release", "check", "cli"]}>
  **The release after the third gauntlet.** `v0.117.0` was tagged from
  `226dc512` and its train failed at startup: the provenance job granted
  the SLSA generator `contents: read` while the generator's nested
  uploader declares `contents: write`, a declaration GitHub validates
  when the reusable workflow is called. No asset shipped under that tag;
  **this patch ships the same tree plus the grant and is the version
  consumers install.** Eight personas ran the 0.116.2
  ...
  These docs teach 0.117.1 (measured 2026-09-02).
</Update>
