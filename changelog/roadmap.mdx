...
<AccordionGroup>
  ...
  <Accordion title="The Connectome era (sovereign memory · the next major, un-numbered until declared)" icon="brain">
    **The first Connectome capabilities land before 1.0** — a declared
    pre-condition of the official launch (the timeline's
    [connectome gate](https://nika.sh/timeline#gates)): recall and ingest
    ship as builtin tools under `invoke:` (never a new verb), the
    zero-LLM write path holds, and store digests are pinned in receipts.
    What follows below is the fuller era beyond those capabilities.
