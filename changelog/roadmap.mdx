---
title: "Roadmap"
description: "Forever-v0.x. No v1.0 target. Quality over speed. First usable ship: v0.81, summer 2026. Milestones tag when truly green."
...
<AccordionGroup>
  <Accordion title="v0.81 — First usable ship (language v0.1 + vertical slice)" icon="rocket">
    **The first public ship — language and engine announced together,
    summer 2026.** The language spec (`nika: v1` envelope, Apache-2.0)
    goes public with a usable vertical slice of the engine: a binary you
    launch, `infer` + `exec` + `nika run` at minimum, `serve`/MCP if
    cadence holds. The rest of the verb surface hardens in v0.8x minors
    after the announce. The communicated version is always the real tag.

    **Engine target state at the tag:**
    ...
    **Summer 2026.** Ships when green, not when the calendar says.
  </Accordion>
  ...
  <Accordion title="The Connectome — sovereign memory" icon="brain">
    **Target state when shipped:**
  ...
  <Accordion title="Ecosystem tier — plugins + observability" icon="box">
    ...
    **No date.** Versions stay honest v0.x tags — there is no marketing
    version axis.
  </Accordion>
