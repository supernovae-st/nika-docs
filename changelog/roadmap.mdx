---
title: "Roadmap"
description: "Real semver toward 1.0. Quality over speed. First public release: 1.0.0, 1 Aug 2026. Milestones tag when truly green."
...
Nika follows **real semver toward 1.0**. The first public release is **1.0.0**
(1 Aug 2026); the current pre-release is {STATUS.version} (release-candidate).
Features arrive in 1.x minors. Each version is a quality milestone earned by
crate admissions + hygiene vector deploys + ADR promotions. When a milestone
is truly green, it tags.
...
<CardGroup cols={2}>
  <Card title="1.0 means we stand behind it" icon="circle-check">
    1.0.0 is the first public release — a usable engine we commit to,
    not a marketing tick. Pre-1.0 the surface is still hardening; 1.0
...
## Current state — v{STATUS.version} (release-candidate)
...
<AccordionGroup>
  <Accordion title="1.0.0 — First public release (language v1 + vertical slice)" icon="rocket">
    **The first public release — language and engine announced together,
    1 Aug 2026.** The language spec (`nika: v1` envelope, Apache-2.0)
    goes public with a usable vertical slice of the engine: a binary you
    launch, `infer` + `exec` + `nika run` at minimum, `serve`/MCP if
    cadence holds. The rest of the verb surface hardens across 1.x minors
