...
<Steps>
  <Step title="Get a first result">
    [Introduction](/) → [SDK quickstart](/sdk/start/quickstart) →
    [project setup](/sdk/start/project-setup). Five minutes inside your
    own app: install the npm package, save one file, run it, read typed
    outputs — no key, no server, no CLI install needed for local runs.
  </Step>
  <Step title="Prefer the terminal?">
    The CLI runs the same files: [install](/getting-started/installation)
    → [first workflow](/getting-started/first-workflow). Same engine,
    same trace.
  </Step>
