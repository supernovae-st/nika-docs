...
| Builtin | What it does |
|---|---|
| `nika:image_generate` | Provider-backed image generation as an *asset pipeline*: images land under a declared `output_dir:` (rides `permits.fs`), outputs and a provenance manifest carry paths + dimensions + sha256 — **image bytes never ride workflow outputs**. Full guide: [Image generation](/guides/image-generation) — sovereign quickstart · cookbook · honesty rules.
