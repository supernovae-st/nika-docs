...
| Builtin | What it does |
|---|---|
| `nika:image_generate` | Provider-backed image generation as an *asset pipeline*: images land under a declared `output_dir:` (rides `permits.fs`), outputs and a provenance manifest carry paths + dimensions + sha256 — **image bytes never ride workflow outputs**. Providers: `openai` (`gpt-image-2`) · `gemini` (`gemini-3.1-flash-image`) · `mock` (offline, deterministic, zero keys). The first graduate of the deferred media class (2026-07-05). |
...
- id: hero
  invoke:
    ...
    args:
      provider: mock              # offline dry-run · flip to gemini/openai for real renders
...
<Warning>
  ...
  `GEMINI_API_KEY` (or the `NIKA_`-prefixed variants) are read once at the
  engine's composition root. The provider endpoints are engine-fixed
