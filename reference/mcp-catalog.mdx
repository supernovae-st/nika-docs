...
<Info>
  **Canonical source:** [`crates/nika-catalog/data/mcp-servers.toml`](https://github.com/supernovae-st/nika/blob/nika-diamond/crates/nika-catalog/data/mcp-servers.toml).
...
tasks:
  - id: crawl
    invoke:
      server: firecrawl        # id from the catalog below
      tool: scrape
      input:
...
# list or run `nika doctor` once the verb-invoke crate is admitted.
...
<Note>
  Verb `invoke:` lands with the `nika-verb-invoke` crate admission
  (Round 7). At v{STATUS.version} the YAML above is the target design —
  see the [constellation](/reference/constellation) for progress.
</Note>
...
env var names are visible in [`mcp-servers.toml`](https://github.com/supernovae-st/nika/blob/nika-diamond/crates/nika-catalog/data/mcp-servers.toml) —
search for `^name = ` under your chosen server.

<Warning>
  Nika never stores API tokens in workflow YAML. Secrets are read from
  environment variables at run time; `nika keys add <env>` saves them in
  your OS keychain (macOS Keychain, Linux Secret Service, Windows
  Credential Manager).
</Warning>
