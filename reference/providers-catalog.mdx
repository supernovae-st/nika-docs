...
<Steps>
  <Step title="Pick your trust model">
    Local / self-hosted (Ollama / LM Studio / llama.cpp / LocalAI / vLLM): zero key, zero egress, sovereign by default. Frontier cloud (OpenAI / xAI / Qwen / Kimi / Anthropic / Google). EU cloud (Mistral). Aggregator (OpenRouter). Each has different residency and compliance trade-offs.
  </Step>
  ...
  <Step title="Verify the backend is reachable">
    Local providers need their server running (e.g. the Ollama daemon on `localhost:11434`) and no key. Cloud providers declare an `env_var` (e.g., `OPENAI_API_KEY`). `nika doctor` checks provider readiness before run.
  </Step>
...
Local/open-weight first (sovereignty default), then cloud — featured
order: OpenAI · xAI · Qwen · Kimi (Moonshot) · Hugging Face · NVIDIA,
...
<Tabs>
  <Tab title="Local / self-hosted (5)">
    ...
    |---|---|---|---|---|
    | `ollama` | Ollama daemon (`localhost:11434`) | `openai-chat` | `(none)` | `qwen3.5:4b` |
    ...
    <Tip>
      **The sovereign default.** No API key, no cloud egress:
      `model: ollama/qwen3.5:4b` runs offline / air-gapped. Any other
      OpenAI-compatible local server (Jan · llamafile · KoboldCpp) routes
      through the `openai` + `base_url` escape hatch.
    </Tip>
  </Tab>

  <Tab title="Cloud (11 canonical)">
    | ID | Name | Dialect | Env var | Default model |
    |---|---|---|---|---|
    | `openai` | OpenAI | `openai-chat` | `OPENAI_API_KEY` | `gpt-5.2` |
    | `xai` | xAI Grok | `openai-chat` | `XAI_API_KEY` | `grok-3` |
    | `qwen` † | Qwen (Alibaba DashScope) | `openai-chat` | `DASHSCOPE_API_KEY` | `qwen-max` |
    | `moonshot` | Moonshot AI (Kimi · frontier open-weight) | `openai-chat` | `MOONSHOT_API_KEY` | `kimi-k2` |
    | `huggingface` | Hugging Face Inference Providers (100+ open-weight · 18 backends) | `openai-chat` | `HF_TOKEN` | `Qwen/Qwen3.5-397B-A17B:fastest` |
    ...
    † `qwen` ships in the engine's extended catalog (one of the
    {STATUS.providers} TOML records), outside the {CANON.providersCloud}
    canonical cloud IDs.
...
tasks:
  primary:
    ...
    infer:
      model: ollama/qwen3.5:4b               # local · zero key
...
```bash Required env
ollama pull qwen3.5:4b          # local primary · no key
export OPENAI_API_KEY=...       # cloud fallback only
...
<CardGroup cols={2}>
  ...
  <Card title="Schema" icon="file-code" href="/reference/schema">
    Workflow envelope, `.nika` top-level keys.
  </Card>
</CardGroup>
