...
2 beats in /home/alice/project/nika.yaml
  ...
  [idle ] workflows/on-event.nika.yaml · on-webhook
         ...
         → no upcoming slot (the clock never fires a webhook beat)
...
journals the gesture, and prints the `launchctl bootout` or `systemctl
--user disable --now` command for the operator; it never executes that
unload command itself.
