...
description: "Visual map of every Nika Diamond crate across six layers. Live admission status. Target 40-42 crates at v0.90."
...
| **L0.5** | nika-kernel, nika-kernel-mock | — | — | Trait definitions + mock companion. Async OK |
...
| **L2** | — | — | verb-exec, verb-fetch, verb-invoke, verb-infer, verb-agent, pck, memory (v0.95), observability, builtin-{github,cloud,workspace} | Verbs + domain services |
...
## Nika Memory (v0.95) — separate count
...
<AccordionGroup>
  <Accordion title="L2 orchestrator" icon="brain">
    `nika-memory` — ties the 8 satellites together behind the `MemoryStore`
...
<Steps>
  ...
  <Step title="Rounds 7-10 — verbs">
    One verb per round: exec → fetch → invoke → infer → agent.
  </Step>
...
<Note>
  ...
  Each admission = 1 atomic commit = 1 `v0.90.0-alpha.N` tag increment.
</Note>
