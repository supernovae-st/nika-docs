...
description: "Visual map of every Nika Diamond crate across six layers. Live admission status. The architecture grows additively across 1.x minors, toward a projected horizon."
...
This page is the **engine crate map**, not the product getting-started
path. The released binary already includes `nika serve`, `nika mcp`,
`nika lsp` and the CLI. The published TypeScript SDK is
`@supernovae-st/nika` — a separate repository from any planned Rust
`nika-sdk` crate on the graph below.

Nika's engine grows additively across 1.x minors, organized across six
layers. Today **{STATUS.cratesAdmitted} crates are admitted** and
{STATUS.wipCrates.length} are WIP
({STATUS.wipCrates.length ? STATUS.wipCrates.join(", ") : "none"}), against a
projected horizon of **{STATUS.cratesTarget} crates** (hard cap
{STATUS.cratesCap}, per ADR-037). The horizon is a projection, never a gate:
...
CLI/LSP/MCP path. Nodes marked 🧭 on the graph are crate-layout
reservations; do not read them as "this command has not shipped."
...
satellites — and is **not counted** in the {STATUS.cratesTarget} crate
horizon. It lands **whole before 1.0** (the
[documented roadmap](/changelog/roadmap) · a declared
...
<AccordionGroup>
  ...
  <Accordion title="L1 satellites (10)" icon="satellite-dish">
...
## Admission queue (historical 0.8x)

This order records how the Diamond rewrite admitted crates. It is **not**
a current product roadmap — `nika-schema`, the four verbs, `nika-cli`,
`nika-mcp`, `nika-lsp` and the `nika` binary already ship. Keep it as
the original sequence (Q5, rev.2):
...
<Steps>
  ...
  <Step title="Round 4: nika-schema">
    ...
    (they need to parse `.nika` step shapes).
  </Step>
