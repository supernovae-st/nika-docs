...
<Info>
  This page is judged by the **downloadable release** — `nika --version`
  must print <code>{STATUS.version}</code> (<code>{STATUS.engineSha}</code>). Catalog counts
  come from `nika catalog --json` on that binary. Language counts come
  from `nika spec --canon` (projected into `CANON`). Crate, test, ADR and
  hygiene totals do not project from the binary and are not on this page.
</Info>
...
<CardGroup cols={3}>
  <Card title="Release" icon="tag">
    **v{STATUS.version}** · <code>{STATUS.engineSha}</code>
  </Card>
  <Card title="Catalog" icon="server">
    **{STATUS.providers}** providers in `nika catalog`
  </Card>
  <Card title="Language" icon="code">
    **{CANON.verbs}** verbs · **{CANON.builtins}** builtins · **{CANON.templates}** skeletons
  </Card>
</CardGroup>

| field | value | projects from |
|---|---|---|
| workspace version | v{STATUS.version} | GitHub latest release / `nika --version` |
| build | {STATUS.engineSha} | `nika --version` parenthetical |
| snapshot day | {STATUS.lastUpdated} | the same release |
| catalog providers | {STATUS.providers} | `nika catalog --json` `.providers.length` |
| spec-named providers | {CANON.providers} | `nika spec --canon` · local {CANON.providersLocal} · cloud {CANON.providersCloud} · test {CANON.providersTest} |
| builtins | {CANON.builtins} | `nika catalog --tools --json` / spec `--canon` |
| skeletons | {CANON.templates} | `nika spec --canon` / `nika compile --list` |

Two nouns, both true, never swapped: the **catalog** is every adapter the
binary can reach; the **spec-named** set is the language stdlib a
conformant engine must ship. See [Providers](/reference/providers-catalog).
