...
| `nika trace ls|verify|reproduce|replay|show|outputs|export|peek|flow` | Prove, replay, summarize, inspect, or export a recorded run — `ls` and `outputs` have `--json` twins (0.117.2) · `peek` prints a fan-out's item table under a value and under the autopsy (0.118) · `verify` walks the tamper-evidence hash chain and names the first broken link (several traces verify per-file, worst exit wins — a shell glob just works) · `peek` renders a task's full recorded value, and a FAILED task's recorded failure + its explain line · `reproduce` re-runs and classifies every divergence — `export` projects the journal to OTLP/JSON lines (Jaeger UI, or POST to any OTLP/HTTP endpoint; a local file, zero collector, zero vendor) |
