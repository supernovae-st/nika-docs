...
| `nika serve` | Resident ARM firer by default (the same `fire`, wall clock in place of the OS). `--bind` with `--workflows` and `--token-file` opens authenticated loopback HTTP: jobs, SSE and OpenAPI. Cancel, artifacts and `POST /v1/run` stay 404 |
