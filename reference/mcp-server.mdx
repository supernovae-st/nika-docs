...
description: "The read-only oracle: 9 validate-and-learn tools over stdio. Agents inspect freely; execution stays on the CLI, where the budget flags and traces live."
...
The released binary serves **stdio** by default. Streamable HTTP also
ships on 0.120.1: `nika mcp --transport http` (POST JSON-RPC,
origin-gated, loopback on port 8123 by default). Widening `--bind`
beyond loopback is on you: put TLS + auth in front first. Either
transport, there is no write tool and no way to make the oracle execute
anything. That is the design, not a gap: generation and repair belong to
[the authoring skill](/getting-started/agents), execution belongs to a
human-gated `nika run`.
...
`nika wire <client>` writes this stanza into the right config file.
Targets on the released binary include cursor · vscode · windsurf ·
claude · claude-desktop · codex · gemini · qwen · opencode · hermes ·
zed · continue · cline · lmstudio · junie · grok · kimi (and
`detected` / `all`). For any other MCP-capable client, paste the stanza
into its server config — see [per-client wiring](/integrations/mcp-clients).
...
| `nika_schema` | The embedded JSON Schema for `*.nika` — the structural contract an agent authors against. |
...
| `nika_inspect` | The typed graph of a checked workflow (`graph_format: 3` · an unwind task is a `finally` node, its edge carries `predicate: unwind`) — nodes, edges with their kinds, byte-parity with `nika inspect --format json` and the LSP. Regenerate a pinned golden from the binary, never hand-edit it. |
...
A `.nika` can declare `exec:` shell steps — that is the language working
...
- What the trace's hash chain proves — and what it does not — is stated
  ...
  for the recorded run, not a signature. Author-binding signatures exist
  (`nika sign`); they do not replace `nika trace verify`.
...
printf '%s\n' \
  '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"probe","version":"1"}}}' \
  '{"jsonrpc":"2.0","method":"notifications/initialized"}' \
  "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{\"name\":\"nika_check\",\"arguments\":{\"workflow\":$(python3 -c 'import json,sys;print(json.dumps(open(sys.argv[1]).read()))' flow.nika)}}}" \
  | nika mcp
```
