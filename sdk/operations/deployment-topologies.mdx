...
|---|---|---|---|
| Product code starts a workflow | local `Nika` | Live in source 0.116 | native process |
| The project clock starts work | `nika serve` (no `--bind`) | Live | `nika.yaml arm[]` to the resident firer |
| An application admits jobs over HTTP | `Nika` + `nika serve --bind` | Live in source/engine 0.116 | Bearer + OpenAPI · cancellation live |
...
OpenAPI `0.116.2` makes cancellation live. Artifact routes, job listing and
