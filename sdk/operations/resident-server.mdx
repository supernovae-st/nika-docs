...
authenticated HTTP. OpenAPI `0.116.2` includes jobs, job SSE, cancellation,
...
`--workflows <dir>` scopes the served registry (0.117.2): the listing, the
...
schedule naming a workflow outside it is refused with the teaching. Every HTTP
job leaves the journal a `nika run` leaves, under the project's
`.nika/traces/` (the trace the job receipt names exists on disk, and
`nika trace verify` seals it) · a cancelled job ends with the one
`execution.cancelled` terminal, never a failure.
