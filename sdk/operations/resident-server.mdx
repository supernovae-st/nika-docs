...
authenticated loopback HTTP. Measured 2026-08-23 on `nika 0.114.0 (b1154df75)`:
`GET /health`, OpenAPI, jobs, job SSE and workflow listing work;
`POST /v1/run`, cancel and artifacts return 404. The binary's OpenAPI says
*Cancel and artifacts are absent.* `--once` and `--dry` cannot bind a listener.

<Warning>
  Bare `nika serve` is the ARM firer, not HTTP. The HTTP door is opt-in
  (`--bind`). Do not call that door “remote execution” without naming the 404s.
  The TypeScript root client is not verified here against this OpenAPI.
</Warning>
