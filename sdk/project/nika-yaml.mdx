...
`nika.yaml` governs the project around the SDK. A `*.nika` file carries
...
arm:
  - workflow: workflows/daily-brief.nika
...
const run = await nika.run('workflows/daily-brief.nika')
...
<Warning>
  ...
  `*.nika` workflow. The engine discovers `nika.yaml` from `cwd` and
