...
Captured verbatim against the released `nika 0.106.0` on a machine with no
keys, no configs, no editors wired — the stranger's first screen:

```text
🦋 nika 0.106.0 — Intent as Code. The workflow language for AI.
...
Captured verbatim against `nika 0.106.0` on a machine with no keys, no
configs, no editors wired — the worst case, and every line hands over:

```text
✔ 9 ok · 17 warn · 0 fail
✔ binary     v0.106.0 · self-contained (spec v1 embedded)
