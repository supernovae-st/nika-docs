...
Captured verbatim against the released `nika 0.99.0` on a machine with no
keys, no configs, no editors wired — the stranger's first screen:

```text
🦋 nika 0.99.0 — Intent as Code. The workflow language for AI.
...
this binary
  4 verbs · 27 builtins · 15 providers · 29 examples · 10 templates
...
`15 providers` are the runnable 10-cloud + 5-local set; the
[catalog](/reference/providers-catalog) counts 16 with the deterministic
`mock`), and key
...
Captured verbatim against `nika 0.99.0` on a machine with no keys, no
configs, no editors wired — the worst case, and every line hands over:

```text
✔ 8 ok · 16 warn · 0 fail
✔ binary     v0.99.0 · self-contained (spec v1 embedded)
