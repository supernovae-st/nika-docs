...
<Warning>
  **Owed a re-capture.** This transcript is a verbatim capture against
  the released `nika 0.105.0`. The rest of this page teaches the 0.106
  grammar; a captured screen is evidence, so it is never hand-edited to
  match — it is re-run. Re-capture it against the 0.106 binary the day the
  tag ships (grep this repo for `Owed a re-capture` to find every one).
</Warning>

Captured verbatim against the released `nika 0.105.0` on a machine with no
keys, no configs, no editors wired — the stranger's first screen:

```text
🦋 nika 0.105.0 — Intent as Code. The workflow language for AI.
   one file · 4 verbs · one binary · audited BEFORE it runs
   every run records a tamper-evident, hash-chained trace

this machine
  editors    cursor ✗ · windsurf ✗ · claude ✗ · zed ✗ · vscode ✗   → nika wire all
...
this binary
  4 verbs · 28 builtins · 15 providers · 33 examples · 10 templates
...
Captured verbatim against `nika 0.105.0` on a machine with no keys, no
configs, no editors wired — the worst case, and every line hands over:

```text
✔ 9 ok · 17 warn · 0 fail
✔ binary     v0.105.0 · self-contained (spec v1 embedded)
...
✔ pricing    606 models · 10 providers · snapshot 2026-07-07 · d31a39603aa5419d · list rates (public catalog)
```
