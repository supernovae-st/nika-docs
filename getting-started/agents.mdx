...
Codex users can skip per-repo setup entirely — the engine repo hosts a
Codex plugin bundling the authoring skill **and** the MCP oracle:
...
One install: the `nika-authoring` skill appears in `/skills` (invoke it
with `$nika-authoring`, or let it auto-trigger on `.nika.yaml` work), and
the read-only oracle (`nika_check` + `nika_explain`) is wired. The plugin
expects the `nika` binary on your PATH
