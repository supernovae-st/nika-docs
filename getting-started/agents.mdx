...
- **`.cursor/rules/nika.mdc`**, the Cursor rules file (globs `*.nika.yaml`):
  ...
  a workflow file.
- **`.agents/skills/nika-authoring/SKILL.md`** (on `main` · next release): a
...
let it auto-trigger on `.nika.yaml` work), and the read-only oracle
(`nika_check` + `nika_explain`) is wired. The plugin expects the `nika`
binary on your PATH (`brew install supernovae-st/tap/nika`).
...
- **`nika_schema` · `nika_examples` · `nika_template` · `nika_canon`**
  (on `main` · ships next release): the authoring surface — the JSON
  Schema, the runnable examples, the 6 skeletons, and the canon SSOT, so
  the agent follows the template→fill→check→repair path instead of
  guessing structure. All read-only, all served from the embedded pack.

<Note>
  The `codex` target is on `main` and ships with the next release. On the
  current release, add the oracle to `~/.codex/config.toml` manually
  (`[mcp_servers.nika]` · `command = "nika"` · `args = ["mcp"]`) — Codex
  also reads `AGENTS.md` natively, so `nika init` alone already teaches it
  the authoring loop.
</Note>
