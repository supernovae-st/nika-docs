...
- **The tool you already pay for.** `nika run --help` names `--access`
  harness on this binary. A pin is a pin: unsatisfied refuses
  (`NIKA-1801`), never substitutes. See
  [Harness access](/guides/harness-access).
...
`nika doctor` reports key *presence* (never values) and which clients
are wired.
