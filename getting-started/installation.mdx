...
<Warning>
  **Latest engine release: v{STATUS.version}** — what `brew install` and the
  install script give you. The npm package
  <code>{SDK.package}@{SDK.sourceVersion}</code> runs on its own channel; it
  bundles its own engine build, so an npm install never depends on your CLI
  version.
  Run `nika --version` and `nika --help` after install; those commands are
  the executable source of truth for your local binary. A grammar change
  ships with its `nika check --fix` migration.
</Warning>
...
<Tabs>
  ...
  <Tab title="curl">
    ...
    Continue with [your first workflow](/getting-started/first-workflow) after
    installation. This documentation is the maintained installation guide.
  </Tab>
...
```bash
nika --version
nika welcome                    # the mirror, and the line to type next
nika compile hello hello.nika   # local starter · explicit dest · zero keys
```

Run `nika welcome` and read the line it offers. **Your** binary is the
source of truth; on v{STATUS.version} that line is
<code>{STATUS.firstCommand}</code>, and this page is gated on saying the same thing.
Piped or in CI, bare `nika` prints the same mirror. On an interactive
terminal, bare `nika` opens the conversational session instead: see
[Describe it, review it, run it](/guides/describe-review-run).
...
<Note>
  **The tool you already pay for** — running a workflow on an agent seat
  ...
  `--access harness` class, compiled into v{STATUS.version}. Its
  prerequisites and limits live in [Harness access](/guides/harness-access);
  `nika doctor` reports what **your** machine can actually use.
</Note>
