...
<Warning>
  **v{STATUS.version} is a pre-1.0 engine release candidate.** The `nika: v1`
  language envelope is stable; the engine and editor surfaces continue to
  harden toward the 1.0 launch.
</Warning>
...
<Tabs>
  ...
  <Tab title="curl">
    ...
    then falls back to the GitHub release binary.
  </Tab>
...
|---|---|---|
| macOS Apple Silicon | `nika-macos-arm64-{STATUS.version}.tar.gz` | shipped |
| macOS Intel | `nika-macos-x64-{STATUS.version}.tar.gz` | shipped |
| Linux x86_64 | `nika-linux-x64-{STATUS.version}.tar.gz` | shipped |
| Linux aarch64 | `nika-linux-arm64-{STATUS.version}.tar.gz` | shipped |
