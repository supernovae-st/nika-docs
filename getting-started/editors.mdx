...
<AccordionGroup>
  <Accordion title="Install = everything on" icon="rocket">
    ...
    diagnostics are live in the same window. No binary? The status menu
    leads with **Finish Setup** — one orchestrated gesture: download →
    wire → optional `nika init` (repo writes stay consented) → recap.
    Fresh workspace? **Run the 10-second proof** (the offline hello,
    zero keys). `nika.autoSetup: false` opts out of all of it.
  </Accordion>
