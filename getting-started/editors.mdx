---
title: "Editor setup"
description: "What IDE integration ships today and what's planned for v0.90+. Schema validation via yaml-language-server works now; full LSP lands with nika-lsp admission."
...
**(2)** a custom `nika-lsp` server (post-v0.90) with cross-task binding
...
## Tier 1 — JSON schema validation (today / v0.90)
...
<Tabs>
  <Tab title="VS Code">
    ...
    ```json settings.json — target v0.90
    {
      "yaml.schemas": {
...
## Tier 2 — `nika-lsp` custom server (post-v0.90)
