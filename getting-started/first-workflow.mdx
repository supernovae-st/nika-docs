...
Write a `.nika` file, run it, get structured output. No setup beyond
...
```bash
nika check hello.nika   # the static audit: plan · cost · secrets · types · before a single token is spent
nika run hello.nika
```
...
<Steps>
  <Step title="Check before running">
    `nika check hello.nika`: static audit before any token or effect.
  </Step>
  <Step title="Inspect the DAG">
    `nika inspect hello.nika` (add `--format mermaid|dot|json` for the machine projections).
  </Step>
