...
- **The re-proof** is `nika trace verify` — it recomputes the chain and
  ...
  trace verb family is in the [CLI reference](/reference/cli)). It
  never starts a run and never proves the business result was correct.
...
From TypeScript (`@supernovae-st/nika@0.120.2`), the same split is
`check()` (the contract), `run()` then `run.result()` (execution), and
`traceVerify()` (the re-proof of existing evidence). A succeeded run
can still be unsealed on a keyless machine.
