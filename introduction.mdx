...
You already have Claude Code or Codex. You do not need an API key. You do
not need to download a model. Install Nika, run one command, see it
finish.
...
v{STATUS.version}: `{STATUS.firstCommand}`. It completes in a second,
offline, with nothing written. The mock echoed the prompt — a rehearsal,
not a billed answer. Keep a file with `nika new snippets/hello`, then
`nika check` and `nika run`.
